...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MyApp.driver = new PL2303Driver(
				(UsbManager) getSystemService(Context.USB_SERVICE), this,
				ACTION_USB_PERMISSION);
		initUI();
		if (!MyApp.driver.PL2303USBFeatureSupported())// 判断系统是否支持USB HOST
		{
			...
			public void onClick(View arg0) {
				if (!isOpen) {
					...
					if (retval == -1)// ResumeUsbList方法用于枚举CH34X设备以及打开相关设备
					{
						...
						if (!MyApp.driver.InitByBaudRate(mBaudrate,700)) {
							if(!MyApp.driver.PL2303Device_IsHasPermission()) {
								Toast.makeText(MainActivity.this, "cannot open, maybe no permission", Toast.LENGTH_SHORT).show();		
							}

							if(MyApp.driver.PL2303Device_IsHasPermission() && (!MyApp.driver.PL2303Device_IsSupportChip())) {
								Toast.makeText(MainActivity.this, "cannot open, maybe this chip has no support, please use PL2303HXD / RA / EA chip.", Toast.LENGTH_SHORT).show();
							}
						} else {        	
...
//				}
		...
		writeButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(null==MyApp.driver)
					return;

				if(!MyApp.driver.isConnected()) 
					return;

				String strWrite = writeText.getText().toString();
				
					Log.d(TAG, "PL2303Driver Write 2(" + strWrite.length() + ") : " + strWrite);
				int res = MyApp.driver.write(strWrite.getBytes(), strWrite.length());
				if( res<0 ) {
					Log.d(TAG, "setup2: fail to controlTransfer: "+ res);
					return;
				} 

				Toast.makeText(MainActivity.this, "Write length: "+strWrite.length()+" bytes", Toast.LENGTH_SHORT).show(); 
...
/*
	...
	public void onResume() {
		...
		if(!MyApp.driver.isConnected()) {
			...
			Log.d(TAG, "New instance : " + MyApp.driver);

			int ret = MyApp.driver.ResumeUsbList();
